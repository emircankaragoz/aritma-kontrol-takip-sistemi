...
export default function GeriDevirHaznesiComponent({ session }) {
    ...
    async function deleteGeriDevirHaznesiVerileri(id) {
        const dataId = {
            dataId: `${id}`,
        };
        const options = {
        ...
        await fetch("/api/controller/post/deleteGeriDevirHaznesi", options)
            .then((res) => res.json())
            .then((data) => {
                if (data) {
        ...
        <div className="container p-2">
            <div className="d-flex  flex-column mx-auto w-50">
                <p className="text-muted text-center fs-5 fw-bolder pb-3 mt-3">
                    GERİ DEVİR HAZNESİ ANALİZ FORMU
                </p>
            ...
            <section>
                <p className="text-muted text-center fs-5 fw-bolder pb-3">
                    GERİ DEVİR HAZNESİ ANALİZ VERİLERİ
                </p>
                        ...
                        <table className="table text-dark table-bordered mt-2">
                            <thead>
                                <tr className="text-center">
                                    ...
                                    <th scope="col">anaerobikHavuzaGeriDonenDebi</th>
                                    <th scope="col">akm</th>
                                    <th scope="col">.</th>
                            ...
                            <tbody className="text-center">
                                {allData.map((data, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{parseFloat(data.anaerobikHavuzaGeriDonenDebi).toFixed(2)}</td>
                                            ...
                                            <td>
                                                <span className="me-2">
                                                    <span
                                                        ...
                                                        onClick={() => deleteGeriDevirHaznesiVerileri(data.id)}
                                                    >
                                                        <RiDeleteBin5Line />
                                                    </span>
                                                </span>
                                                <span>
                                                    <GeriDevirHaznesiUpdateModal dataId={data.id}/>
                                                </span>
