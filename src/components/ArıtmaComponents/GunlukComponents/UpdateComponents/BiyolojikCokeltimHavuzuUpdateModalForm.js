...
export default function ModalForm({ dataId }) {
    ...
    async function getAllBiyolojikCokeltimHavuzuDataHandler() {
        if (dataId !== undefined && dataId !== null) {
            await aritmaService.getBiyolojikCokeltimHavuzuById(dataId)
                .then((result) => {
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            amonyumAzotu: `${allDataById != undefined ? allDataById.amonyumAzotu : ""}`,
    ...
    async function onSubmit(values) {
        ...
        await fetch("/api/controller/post/updateBiyolojikCokeltimHavuzu", options)
            .then((res) => res.json())
            .then((data) => {
                if (data) {
            ...
            <div>
                <form onSubmit={formik.handleSubmit}>
                    <div className="form-group py-2">
                        ...
                        <input className="form-control"
                            type="text"
                            defaultValue={allDataById.amonyumAzotu || ""}
                            name="amonyumAzotu"
