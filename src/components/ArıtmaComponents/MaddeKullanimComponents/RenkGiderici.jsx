...
import RenkGidericiUpdateModal from "./RenkGidericiUpdateModal";

export default function RenkGidericiPage({ session }) {
  const router = useRouter();

  const [isDataEntered, setIsDataEntered] = useState(null);
  const [getAllRenkGiderici, setAllRenkGiderici] = useState([]);
  ...
  async function getAllRenkGidericiKullanimHandler() {
    await aritmaService.getAllRenkGidericiKullanim().then((result) => {
      setAllRenkGiderici(result.data);
  ...
  async function createdSystemMessageHandler(date) {
    await systemMessageService.addSystemMessage(
      SYSTEM_MESSAGES.M3.content,
      SYSTEM_MESSAGES.M3.title,
      SYSTEM_MESSAGES.M3.code,
      date
    );
  }
  ...
  useEffect(() => {
    return () => {
      getAllRenkGidericiKullanimHandler();
  ...
  async function deleteAritma(id) {
    ...
    await fetch("/api/controller/post/deleteRenkGidericiKullanim", options)
      .then((res) => res.json())
      .then((data) => {
        if (data) {
          router.refresh();
    ...
    <div className="container p-2">
      <div className="d-flex  flex-column mx-auto w-50">
        <p className="text-muted text-center fs-5 fw-bolder pb-3 mt-3">
          Renk Giderici Kullanım Miktarları ve Maliyeti
        </p>
      ...
      <section>
        <p className="text-muted text-center fs-5 fw-bolder pb-3">
          Tüm Renk Giderici Kullanım Miktarları
        </p>
        <div className="row">
          <div className="col-sm-12 table-responsive">
            <table className="table table-sm  table-bordered text-dark mt-2">
              ...
              <tbody className="text-center">
                {getAllRenkGiderici.map((aritma, index) => (
                  <tr key={index}>
                    ...
                    <td>{parseFloat(aritma.birimFiyatEuro).toFixed(2)}</td>
                    <td>
                      {parseFloat(
                        (
                          aritma.kimyasalYogunluk *
                          (aritma.baslangicLt - aritma.bitisLt) *
                          aritma.birimFiyatEuro
                        ).toFixed(2)
                      )}
                    </td>
