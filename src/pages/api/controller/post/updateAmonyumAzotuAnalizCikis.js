...
export default async function handler(req, res) {
    ...
    if (req.method === "POST") {
        ...
        const checkexisting = await prisma.amonyumAzotuAnalizCikis.findUnique({
            where: { id: ID },
            select: {
                id: true,
            },
        ...
        if (checkexisting !== null) {
            ...
            try {
                const data = await prisma.amonyumAzotuAnalizCikis.update({
                    where: {
                        id: ID,
                    },
                    data: {
                ...
                res.status(201).json({ status: true, amonyumAzotuAnalizCikis: data });
            } catch (err) {
